...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(final float partialTicks) {
        ...
        ItemRenderer.partTicks = partialTicks;
        ...
        if (this.itemToRender != null) {
            final Module killAura = Astomero.instance.moduleManager.getModuleByName("KillAura");
            final ModeSet blockMode = Astomero.instance.moduleManager.getModeSetByName("KillAura", "AB Mode");
            final boolean isKillAuraAttacking = KillAura.target != null;
            final boolean holdingSword = this.mc.thePlayer.getHeldItem() != null && this.mc.thePlayer.getHeldItem().getItem() instanceof ItemSword;
            final boolean fakeBlock = killAura.isToggled() && (blockMode.is("Fake") || blockMode.is("Verus")) && isKillAuraAttacking && holdingSword;
            if (this.itemToRender.getItem() == Items.filled_map) {
                this.renderItemMap(entityplayersp, f3, f, f2);
            }
            else if (entityplayersp.getItemInUseCount() > 0 || fakeBlock) {
                ...
                switch (ItemRenderer$1.field_178094_a[enumaction.ordinal()]) {
                    ...
                    case 4: {
                        ...
                        float var10 = MathHelper.sin(MathHelper.sqrt_float(f2) * 3.1415927f);
                        final float f5 = MathHelper.sin((float)(MathHelper.sqrt_float(f2) * 3.1));
                        if (ModuleManager.getModule(Animations.class).isEnabled()) {
                            final ModeSet anim = Astomero.instance.moduleManager.getModeSetByName("Animations", "Mode");
                            if (anim.is("Exhibition")) {
                            ...
                            if (anim.is("Spin")) {
                                ...
                                if (this.rotateTimer.hasReached(1L)) {
                                    for (int i = 0; i < Animations.spinSpeed.getValue(); ++i) {
                                        ++this.delay;
                                    }
                            ...
                            if (anim.is("Spinny")) {
                                GlStateManager.translate(0.0, 0.1, -0.1);
                                this.transformFirstPersonItem(f / 2.0f, 0.0f);
                                GlStateManager.rotate(40.0f, -0.0f, 0.0f, -0.05f);
    ...
    private void old(final float equipProgress, final float swingProgress) {
