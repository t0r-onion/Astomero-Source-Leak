...
import com.initial.notifications.*;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(final float partialTicks) {
        final EventRenderOverlay eventRenderOverlay = new EventRenderOverlay();
        eventRenderOverlay.call();
        ...
        final Event2D event2D = new Event2D((float)scaledresolution.getScaledWidth(), (float)scaledresolution.getScaledHeight());
        event2D.call();
        ...
        if (scoreobjective2 != null) {
            final EventRenderScoreboard event = new EventRenderScoreboard(Event.State.PRE);
            event.call();
            this.renderScoreboard(scoreobjective2, scaledresolution);
            final EventRenderScoreboard event2 = new EventRenderScoreboard(Event.State.POST);
            event2.call();
        ...
        final EventRender2D e = new EventRender2D(scaledresolution, partialTicks, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight());
        e.call();
        final EventRenderUI eventRenderUI = new EventRenderUI();
        eventRenderUI.call();
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    public void renderDemo(final ScaledResolution p_175185_1_) {
        ...
        this.getFontRenderer().drawStringWithShadow(s, (float)(p_175185_1_.getScaledWidth() - i - 10), 5.0, 16777215);
        this.mc.mcProfiler.endSection();
    }
    
    protected boolean showCrosshair() {
        final EventCrosshair eventCrosshair = new EventCrosshair();
        eventCrosshair.call();
