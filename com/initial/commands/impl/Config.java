        ...
        if (args.length > 1) {
            ...
            switch (s) {
                ...
                case "delete": {
                    try {
                        ...
                        Astomero.addChatMessage("Nigga Java just killed itself, contact da owner");
                    }
                    finally {
            ...
            try {
                ...
                if (type.equalsIgnoreCase("list")) {
                    ...
                    Astomero.addChatMessage("Cfg List:");
                    Astomero.addChatMessage("");
                    try {
                        for (final File archivogay : Objects.requireNonNull(Objects.requireNonNull(dir).listFiles())) {
                            final String fileName = archivogay.getName().substring(0, archivogay.getName().length() - 5);
                            Astomero.addChatMessage(fileName);
                        }
                    }
                    catch (NullPointerException exc) {
                        Astomero.addChatMessage("An exception made Java kill itself.");
                    }
                    Astomero.addChatMessage("");
                }
            }
